...
function findElement(xml: string, tag: string): { readonly inner: string; readonly raw: string } | undefined {
  const open = `<${tag}`
  const close = `</${tag}>`
  let from = 0
  for (;;) {
 ...
 */
export function readDocxText(zip: Map<string, ZipEntryData>): string {
  const entry = zip.get(DOCUMENT_PART)
  if (!entry) throw new Error(`DOCX 缺少主文档部件 ${DOCUMENT_PART}`)
...
function runXml(text: string, bold: boolean, halfPoints: number | undefined, centered: boolean): string {
  const props: string[] = []
  if (centered) props.push('<w:jc w:val="center"/>')
  if (bold) props.push('<w:b/>')
  if (halfPoints !== undefined) props.push(`<w:sz w:val="${halfPoints}"/>`)
...
function paragraphXml(text: string, options?: { readonly bold?: boolean; readonly halfPoints?: number; readonly centered?: boolean; readonly style?: string }): string {
 ...
 */
export function buildDocx(input: { readonly title?: string; readonly paragraphs: readonly string[] }): Uint8Array {
...
function encodeAscii(xml: string): Uint8Array {
  ...
  for (let index = 0; index < xml.length; index++) {
    ...
    if (code > 0x7f) throw new Error(`DOCX 模板出现非 ASCII 字符（位置 ${index}），请改用 escapeXmlText 转义`)
    bytes[index] = code
  }
  return bytes
}

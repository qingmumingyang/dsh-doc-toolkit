...
const i16 = (b: Buffer, o: number): number => b.readInt16BE(o)
const u32 = (b: Buffer, o: number): number => b.readUInt32BE(o)

/** TrueType 表校验和：按大端 uint32 求和，不足 4 字节补零。 */
function tableChecksum(buf: Buffer): number {
  let sum = 0
  const n = Math.floor(buf.length / 4)
  for (let i = 0; i < n; i++) sum = (sum + u32(buf, i * 4)) >>> 0
  if (buf.length % 4 > 0) {
...
// ============================================================
// TrueType / TrueType Collection 解析
// ============================================================
 ...
 */
export function parseFont(buf: Buffer, fontIndex = 0): ParsedFont {
  const tag = buf.subarray(0, 4).toString('latin1')
  let base = 0
  if (tag === 'ttcf') {
    const numFonts = u32(buf, 8)
    if (fontIndex >= numFonts) throw new Error(`TTC 字体索引越界: ${fontIndex}/${numFonts}`)
    base = u32(buf, 12 + fontIndex * 4)
  }
  if (u16(buf, base) !== 0x0001) throw new Error('不是有效的 TrueType 字体')
  ...
  const need = (tagName: string): Buffer => {
    ...
    if (!t) throw new Error(`字体缺少 ${tagName} 表`)
  ...
  if (u32(maxp, 0) === 0x00005000) throw new Error('OTF/CFF 字体暂不支持（仅支持 TrueType/TTC）')
  ...
  if (indexToLocFormat === 0) {
    for (let i = 0; i <= numGlyphs; i++) loca.push(u16(locaTable, i * 2) * 2)
  } else {
    for (let i = 0; i <= numGlyphs; i++) loca.push(u32(locaTable, i * 4))
  }

  const numberOfHMetrics = u16(hhea, 34)
  const hmtxTable = need('hmtx')
  const advances: number[] = []
  for (let i = 0; i < numberOfHMetrics; i++) advances.push(u16(hmtxTable, i * 4))
  const lastAdvance = advances[advances.length - 1] ?? 0
  for (let i = numberOfHMetrics; i < numGlyphs; i++) advances.push(lastAdvance)
...
function parseCmap(cmap: Buffer): Map<number, number> {
  ...
  const order: Array<[number, number]> = [[3, 1], [0, 3], [3, 10], [0, 4]]
  const merged = new Map<number, number>()
  for (const [p, e] of order) {
    const sub = subs.find((s) => s.platform === p && s.encoding === e)
    if (sub) for (const [c, g] of sub.map) merged.set(c, g)
  }
  if (merged.size === 0) {
    for (const sub of subs) for (const [c, g] of sub.map) if (!merged.has(c)) merged.set(c, g)
  }
  return merged
}
 ...
 */
export function buildSubset(font: ParsedFont, usedChars: number[]): Buffer {
  const usedGids = new Set<number>([0])
  for (const c of usedChars) {
    const g = font.charToGid.get(c)
    if (g !== undefined) usedGids.add(g)
  }
  ...
  while (changed) {
    ...
    for (const gid of [...usedGids]) {
      ...
      if (len < 10) continue
      if (i16(font.glyf, off) >= 0) continue
      let p = off + 10
      const end = off + len
      while (p + 4 <= end) {
        ...
        p += 4
        if (flags & 0x0001) p += 2 // ARG_1_AND_2_ARE_WORDS
  ...
  for (let i = 0; i < glyphs.length; i++) {
    if (long) loca.writeUInt32BE(acc, i * 4)
    else loca.writeUInt16BE(acc / 2, i * 2)
    acc += glyphs[i].length
  }
  if (long) loca.writeUInt32BE(acc, numGlyphs * 4)
  else loca.writeUInt16BE(acc / 2, numGlyphs * 2)
  ...
  if (font.os2) tables.push(['OS/2', font.os2])
  if (font.post) tables.push(['post', font.post])
  if (font.name) tables.push(['name', font.name])
  return assembleTtf(tables)
}

/** 重建 cmap：format 4（BMP）+ 必要时 format 12（含非 BMP）。 */
function buildCmap(usedChars: number[], font: ParsedFont): Buffer {
  ...
  const subs: Array<[number, number, Buffer]> = [[3, 1, sub4], [0, 3, sub4]]
  if (nonBmp.length > 0) {
...
function buildCmap4(sortedBmp: number[], font: ParsedFont): Buffer {
...
function buildCmap12(sorted: number[], font: ParsedFont): Buffer {
...
function assembleTtf(tables: Array<[string, Buffer]>): Buffer {
...
function helveticaWidth1000(ch: string): number {
  const c = ch.charCodeAt(0)
  if (c >= 32 && c <= 126) return HELVETICA_WIDTHS[c - 32]
  return 556
}
...
function makeHelveticaFace(): FontFace {
  return {
    ref: 'F1',
    codeHex(text: string): string {
      let hex = ''
...
class DocLayout {
  ...
  private readonly boldRef: string
  private readonly titleSize = 18
  private readonly paraSize = 11
  private readonly cellSize = 9.5

  constructor(face: FontFace, boldRef: string) {
    this.face = face
  ...
  private emit(op: string): void {
    this.pages[this.pages.length - 1] += op
  }

  private lineHeight(fs: number): number {
    return fs * LINE_SCALE
  }

  private textWidth(text: string, fs: number): number {
    let w = 0
    for (const ch of text) w += (this.face.width1000(ch) / 1000) * fs
    return w
  }

  /** 按字符宽度折行，保留显式换行。 */
  private wrap(text: string, fs: number, maxWidth: number): string[] {
    const lines: string[] = []
    let cur = ''
    let curW = 0
  ...
  private drawLine(text: string, fs: number, x: number, baselineY: number, bold: boolean): void {
  ...
  private measureRow(row: string[], widths: number[], fs: number, lh: number, padX: number, padY: number): number {
    let nLines = 1
  ...
  /** 在 yTop（行顶）绘制一行，返回行高。 */
  private renderRowAt(row: string[], widths: number[], yTop: number, isHeader: boolean, fs: number, lh: number, padX: number, padY: number): number {
  ...
  table(rows: string[][]): void {
    ...
    for (let c = 0; c < colCount; c++) {
      ...
      for (const row of rows) {
        let w = 0
        for (const ch of (row[c] ?? '').replace(/\t/g, '    ')) w += (this.face.width1000(ch) / 1000) * fs
        if (w > maxW) maxW = w
      }
    ...
    if (total > USABLE_W) {
      ...
      for (let c = 0; c < colCount; c++) widths[c] = Math.max(20, widths[c] * scale)
    }
...
class PdfBuilder {
  ...
  build(root: number, info: number): Buffer {
...
function escapeInfoString(s: string): string {
  if (/[^\x20-\x7e]/.test(s)) {
...
function buildWidthsArray(emb: EmbeddedFont): string {
  ...
  while (i < gids.length) {
    ...
    const ws = gids.slice(i, j + 1).map((g) =>
      Math.round((emb.parsed.advances[Math.min(g, emb.parsed.advances.length - 1)] * 1000) / emb.parsed.unitsPerEm)
    )
...
function buildToUnicode(emb: EmbeddedFont): string {
...
function assemblePdf(pages: string[], emb: EmbeddedFont | undefined, meta: PdfMeta): Buffer {
...
export async function writePDF(filePath: string, content: Record<string, unknown>, signal?: AbortSignal): Promise<string> {
  ...
  if (Array.isArray(content.rows)) {
    rows = (content.rows as unknown[][]).map((row) => (row as unknown[]).map((cell) => (cell === null || cell === undefined ? '' : String(cell))))
  }
  ...
  let emb: EmbeddedFont | undefined
  let face: FontFace
  let boldRef = 'F2'
  ...
  for (const p of paragraphs) layout.paragraph(p)

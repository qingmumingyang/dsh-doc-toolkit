...
function latin1(bytes: Uint8Array, start: number, end: number): string {
  ...
  for (let index = start; index < stop; index += 1) out += String.fromCharCode(bytes[index]!)
  return out
}

/** 在字节数组里查找 ASCII 串，找不到返回 -1。 */
function indexOfText(bytes: Uint8Array, needle: string, from = 0): number {
  ...
  for (let index = Math.max(0, from); index <= limit; index += 1) {
    ...
    for (let offset = 1; offset < needle.length; offset += 1) {
      if (bytes[index + offset] !== needle.charCodeAt(offset)) { matched = false; break }
    }
...
function lastIndexOfText(bytes: Uint8Array, needle: string, from?: number): number {
...
type PdfValue =
  ...
  | { readonly k: 'bool'; readonly b: boolean }
  | { readonly k: 'null' }
  | { readonly k: 'name'; readonly s: string }
  | { readonly k: 'str'; readonly bytes: Uint8Array }
  | { readonly k: 'arr'; readonly items: PdfValue[] }
  | { readonly k: 'dict'; readonly map: Map<string, PdfValue> }
  | { readonly k: 'ref'; readonly num: number; readonly gen: number }
  | { readonly k: 'stream'; readonly map: Map<string, PdfValue>; readonly data: Uint8Array }

const NULL_VALUE: PdfValue = { k: 'null' }

/** 解析游标：在字节数组上前后移动。 */
class Cursor {
  constructor(readonly bytes: Uint8Array, public pos = 0) {}
  atEnd(): boolean { return this.pos >= this.bytes.length }
  peek(): number { return this.pos < this.bytes.length ? this.bytes[this.pos]! : -1 }
}
...
function skipSpace(cursor: Cursor): void {
  ...
  while (cursor.pos < bytes.length) {
    ...
    if (isWhite(byte)) { cursor.pos += 1; continue }
    if (byte === 0x25) { // '%'
      while (cursor.pos < bytes.length && bytes[cursor.pos] !== CH_LF && bytes[cursor.pos] !== CH_CR) cursor.pos += 1
      continue
    }
    break
  }
...
function readRegular(cursor: Cursor): string {
  const start = cursor.pos
  while (cursor.pos < cursor.bytes.length && isRegular(cursor.bytes[cursor.pos]!)) cursor.pos += 1
...
function parseName(cursor: Cursor): string {
  ...
  // `#xx` 十六进制转义
  return raw.replace(/#([0-9A-Fa-f]{2})/g, (_all, hex: string) => String.fromCharCode(Number.parseInt(hex, 16)))
}

/** 解析字面字符串 `(...)`，处理转义与嵌套括号。 */
function parseLiteralString(cursor: Cursor): Uint8Array {
  cursor.pos += 1 // '('
  const out: number[] = []
  let depth = 1
  const { bytes } = cursor
  while (cursor.pos < bytes.length) {
    ...
    if (byte === 0x5c) { // 反斜杠
      ...
      if (next === CH_LF) { cursor.pos += 1; continue }
      if (next === CH_CR) { cursor.pos += 1; if (bytes[cursor.pos] === CH_LF) cursor.pos += 1; continue }
      ...
      const simple: Record<string, number> = { n: CH_LF, r: CH_CR, t: CH_TAB, b: 0x08, f: CH_FF }
      const mapped = simple[String.fromCharCode(next)]
      out.push(mapped === undefined ? next : mapped)
      cursor.pos += 1
      continue
    }
    if (byte === 0x28) { depth += 1; out.push(byte); cursor.pos += 1; continue }
    if (byte === 0x29) {
      depth -= 1
      cursor.pos += 1
      if (depth === 0) break
...
function parseHexString(cursor: Cursor): Uint8Array {
  cursor.pos += 1 // '<'
  const digits: number[] = []
  const { bytes } = cursor
  while (cursor.pos < bytes.length) {
...
function lookupNumberStart(cursor: Cursor): { value: number; next: number } | undefined {
  const save = cursor.pos
  skipSpace(cursor)
  const start = cursor.pos
  const token = readRegular(cursor)
  if (!/^[+-]?(?:\d+\.?\d*|\.\d+)$/.test(token)) { cursor.pos = save; return undefined }
  return { value: Number(token), next: start }
}

/** 解析一个完整对象（不含 `obj`/`endobj` 包裹）。 */
function parseValue(cursor: Cursor, depth = 0): PdfValue {
  if (depth > 64) return NULL_VALUE
  skipSpace(cursor)
  const byte = cursor.peek()
  if (byte < 0) return NULL_VALUE

  if (byte === 0x2f) return { k: 'name', s: parseName(cursor) }
  if (byte === 0x28) return { k: 'str', bytes: parseLiteralString(cursor) }
  if (byte === 0x5b) { // '['
    cursor.pos += 1
    ...
    for (;;) {
      ...
      if (cursor.peek() === 0x5d) { cursor.pos += 1; break }
      const before = cursor.pos
      items.push(parseValue(cursor, depth + 1))
      if (cursor.pos === before) { cursor.pos += 1 } // 防御：绝不原地打转
  ...
  if (byte === 0x3c) { // '<'
    if (cursor.bytes[cursor.pos + 1] === 0x3c) {
      ...
      for (;;) {
        skipSpace(cursor)
        if (cursor.atEnd()) break
        if (cursor.peek() === 0x3e && cursor.bytes[cursor.pos + 1] === 0x3e) { cursor.pos += 2; break }
        if (cursor.peek() !== 0x2f) {
  ...
  const save = cursor.pos
  const token = readRegular(cursor)
  if (token === '') { cursor.pos = save + 1; return NULL_VALUE }
  if (token === 'true') return { k: 'bool', b: true }
  if (token === 'false') return { k: 'bool', b: false }
  ...
  if (/^[+-]?(?:\d+\.?\d*|\.\d+)$/.test(token)) {
    ...
    if (second !== undefined) {
      ...
      if (keyword === 'R') return { k: 'ref', num: Number(token), gen: second.value }
...
/** 交叉引用条目。 */
type XrefEntry = { readonly kind: 1; readonly offset: number } | { readonly kind: 2; readonly streamNum: number; readonly index: number }

/** 一份已加载的 PDF。 */
class PdfDocument {
  readonly cache = new Map<number, PdfValue>()
  readonly xref = new Map<number, XrefEntry>()
  readonly trailer = new Map<string, PdfValue>()
  private readonly objectStreams = new Map<number, Map<number, PdfValue>>()
  private scanOffsets: Map<number, number> | undefined
  private parsedObjects = 0

  constructor(readonly bytes: Uint8Array, private readonly maxObjects: number) {}

  private countObject(): boolean {
  ...
  getObject(num: number): PdfValue | undefined {
    ...
    if (!this.countObject()) return undefined

    const entry = this.xref.get(num)
    let value: PdfValue | undefined
    if (entry?.kind === 1) value = this.parseObjectAt(entry.offset, num)
    else if (entry?.kind === 2) value = this.objectStreams.get(entry.streamNum)?.get(num) ?? this.loadObjectStream(entry.streamNum)?.get(num)

    if (value === undefined) {
      // 交叉引用不可信（损坏或被裁剪）：回退到全文件扫描。
      const offsets = this.scanIndex()
      const fallback = offsets.get(num)
      if (fallback !== undefined) value = this.parseObjectAt(fallback, num)
    }
  ...
  resolve(value: PdfValue | undefined): PdfValue | undefined {
    let current = value
    for (let hops = 0; hops < 32 && current?.k === 'ref'; hops += 1) current = this.getObject(current.num)
    return current
  }

  /** 解析字典（自动解引用）。 */
  resolveDict(value: PdfValue | undefined): Map<string, PdfValue> | undefined {
    const resolved = this.resolve(value)
    return resolved?.k === 'dict' ? resolved.map : undefined
  }

  /** 读取字典里的数字字段（自动解引用）。 */
  numberField(map: Map<string, PdfValue> | undefined, key: string): number | undefined {
  ...
  /** 读取字典里的名字字段（自动解引用）。 */
  nameField(map: Map<string, PdfValue> | undefined, key: string): string | undefined {
    if (map === undefined) return undefined
    const resolved = this.resolve(map.get(key))
  ...
  /** 在指定偏移解析间接对象，并校验对象号。 */
  private parseObjectAt(offset: number, expected: number): PdfValue | undefined {
    ...
    if (head === undefined || head.value !== expected) return undefined
    const gen = lookupNumberStart(cursor)
    if (gen === undefined) return undefined
    if (readToken(cursor) !== 'obj') return undefined

    const value = parseValue(cursor)
    if (value.k !== 'dict') return value

    // 可能是流：`stream` 关键字必须紧跟字典。
    const save = cursor.pos
    if (readToken(cursor) !== 'stream') { cursor.pos = save; return value }
    // 流数据从 `stream` 后的行尾开始
    if (this.bytes[cursor.pos] === CH_CR) cursor.pos += 1
    if (this.bytes[cursor.pos] === CH_LF) cursor.pos += 1
    const start = cursor.pos
  ...
  private scanIndex(): Map<number, number> {
    if (this.scanOffsets !== undefined) return this.scanOffsets
    const offsets = new Map<number, number>()
    const bytes = this.bytes
    for (let index = 0; index + 3 < bytes.length; index += 1) {
      if (bytes[index] !== 0x6f || bytes[index + 1] !== 0x62 || bytes[index + 2] !== 0x6a) continue // 'obj'
      if (index > 0 && isRegular(bytes[index - 1]!)) continue
      // 往回读 `<num> <gen> obj`
      let back = index - 1
      while (back >= 0 && isWhite(bytes[back]!)) back -= 1
      const genEnd = back + 1
      while (back >= 0 && bytes[back]! >= 0x30 && bytes[back]! <= 0x39) back -= 1
      ...
      if (!Number.isSafeInteger(num) || num <= 0 || offsets.has(num)) continue
  ...
  /** 加载一个对象流，返回其中所有对象。 */
  private loadObjectStream(streamNum: number): Map<number, PdfValue> | undefined {
    const existing = this.objectStreams.get(streamNum)
    if (existing !== undefined) return existing

    const stream = this.resolve({ k: 'ref', num: streamNum, gen: 0 })
    if (stream?.k !== 'stream') return undefined
    if (this.nameField(stream.map, 'Type') !== 'ObjStm') return undefined
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (num === undefined || offset === undefined || !Number.isFinite(num) || !Number.isFinite(offset)) break
      const cursor = new Cursor(data, Math.min(first + offset, data.length))
      objects.set(num, parseValue(cursor))
      if (objects.size > 100_000) break
    }
  ...
  decodeStream(stream: { map: Map<string, PdfValue>; data: Uint8Array }): Uint8Array | undefined {
    const filters: string[] = []
    const filterValue = this.resolve(stream.map.get('Filter'))
    if (filterValue?.k === 'name') filters.push(filterValue.s)
    else if (filterValue?.k === 'arr') {
      for (const item of filterValue.items) {
        const resolved = this.resolve(item)
        if (resolved?.k === 'name') filters.push(resolved.s)
        else if (resolved?.k === 'arr' && resolved.items.length > 0) {
          const first = this.resolve(resolved.items[0])
          if (first?.k === 'name') filters.push(first.s) // [/FlateDecode /Predictor] 旧式写法
        }
      }
    }

    const parmsValue = this.resolve(stream.map.get('DecodeParms'))
    const parmsList: (Map<string, PdfValue> | undefined)[] = []
    if (parmsValue?.k === 'dict') parmsList.push(parmsValue.map)
    else if (parmsValue?.k === 'arr') for (const item of parmsValue.items) parmsList.push(this.resolveDict(item))
    ...
    for (let index = 0; index < filters.length; index += 1) {
      ...
      if (filter === 'ASCIIHexDecode' || filter === 'AHx') { data = asciiHexDecode(data); continue }
  ...
  catalog(): Map<string, PdfValue> | undefined {
    ...
    for (const num of this.objectNumbers()) {
      ...
      if (dict !== undefined && this.nameField(dict, 'Type') === 'Catalog') return dict
    }
    return undefined
  }
}
...
function inflate(data: Uint8Array): Uint8Array | undefined {
  try { return new Uint8Array(inflateSync(data)) } catch { /* 继续尝试 */ }
  try { return new Uint8Array(inflateRawSync(data)) } catch { return undefined }
}
...
/** ASCII85 解码。 */
function ascii85Decode(data: Uint8Array): Uint8Array | undefined {
  const out: number[] = []
  let group: number[] = []
  for (let index = 0; index < data.length; index += 1) {
    const byte = data[index]!
    if (isWhite(byte)) continue
    if (byte === 0x7e) break // '~' 之后的 '>' 是结束标记
    if (byte === 0x7a && group.length === 0) { out.push(0, 0, 0, 0); continue } // 'z'
    if (byte < 0x21 || byte > 0x75) return undefined
  ...
  if (group.length > 1) {
    ...
    let value = 0
    for (let index = 0; index < 5; index += 1) value = value * 85 + (group[index] ?? 84)
...
function readXrefSection(doc: PdfDocument, offset: number, depth: number): boolean {
  if (depth > 32 || offset < 0 || offset >= doc.bytes.length) return false
    ...
    for (;;) {
      ...
      if (peek === 0x74) { // 't' → trailer
        ...
        if (trailerValue.k === 'dict') {
          for (const [key, value] of trailerValue.map) if (!doc.trailer.has(key)) doc.trailer.set(key, value)
          const prev = doc.numberField(trailerValue.map, 'Prev')
          if (prev !== undefined) readXrefSection(doc, prev, depth + 1)
        }
        return true
      }
      if (!/[0-9]/.test(String.fromCharCode(peek))) break
      const start = Number(readToken(cursor))
      const count = Number(readToken(cursor))
      if (!Number.isFinite(start) || !Number.isFinite(count)) break
      for (let index = 0; index < count; index += 1) {
        const entryOffset = Number(readToken(cursor))
        skipSpace(cursor)
        const gen = Number(readToken(cursor))
        const flag = readToken(cursor)
        if (!Number.isFinite(entryOffset)) return false
        const num = start + index
        if (flag === 'n' && !doc.xref.has(num)) doc.xref.set(num, { kind: 1, offset: entryOffset })
        void gen
      }
    }
    return true
  ...
  if (readToken(cursor) !== 'obj') return false
  const value = parseValue(cursor)
  if (value.k !== 'dict') return false
  if (doc.nameField(value.map, 'Type') !== 'XRef') return false
  // 读流数据
  const save = cursor.pos
  if (readToken(cursor) !== 'stream') { cursor.pos = save; return false }
  if (doc.bytes[cursor.pos] === CH_CR) cursor.pos += 1
  if (doc.bytes[cursor.pos] === CH_LF) cursor.pos += 1
  const dataStart = cursor.pos
  const declaredLength = doc.numberField(value.map, 'Length') ?? 0
  let stream: { map: Map<string, PdfValue>; data: Uint8Array } = {
    map: value.map,
  ...
  if (widths?.k !== 'arr' || widths.items.length < 3) return false
  ...
  for (let range = 0; range + 1 < ranges.length; range += 2) {
    ...
    for (let index = 0; index < count; index += 1) {
      if (position + rowLength > decoded.length) break
      const fields = readXrefFields(decoded, position, widthsNumbers)
      position += rowLength
      const num = start + index
      const type = widthsNumbers[0] === 0 ? 1 : fields[0]!
      if (doc.xref.has(num)) continue
      if (type === 1) doc.xref.set(num, { kind: 1, offset: fields[1]! })
      else if (type === 2) doc.xref.set(num, { kind: 2, streamNum: fields[1]!, index: fields[2]! })
    }
  }

  for (const [key, item] of value.map) if (!doc.trailer.has(key)) doc.trailer.set(key, item)
...
/** 按 /W 宽度读一条交叉引用记录。 */
function readXrefFields(data: Uint8Array, offset: number, widths: readonly number[]): number[] {
  const fields: number[] = []
  let position = offset
  for (const width of widths) {
    let value = 0
    for (let index = 0; index < width; index += 1) value = value * 256 + (data[position + index] ?? 0)
    position += width
    fields.push(value)
  }
  return fields
...
function loadXref(doc: PdfDocument): boolean {
  ...
  if (marker >= 0) {
    ...
    if (Number.isFinite(offset) && readXrefSection(doc, offset, 0)) return true
  }
  ...
  if (trailerAt >= 0) {
    ...
    const value = parseValue(cursor)
    if (value.k === 'dict') {
      for (const [key, item] of value.map) doc.trailer.set(key, item)
      const root = doc.resolve(value.map.get('Root'))
      if (root?.k === 'ref') {
        const prev = doc.numberField(value.map, 'Prev')
        if (prev !== undefined) readXrefSection(doc, prev, 0)
        return true
      }
    }
  }
...
function winAnsi(code: number): number {
...
function glyphToUnicode(name: string): number | undefined {
  const uni = /^uni([0-9A-Fa-f]{4})/.exec(name)
  if (uni?.[1] !== undefined) return Number.parseInt(uni[1], 16)
  const u = /^u([0-9A-Fa-f]{4,6})$/.exec(name)
  if (u?.[1] !== undefined) return Number.parseInt(u[1], 16)
  if (name === '.notdef' || name === '') return undefined
  const mapped = GLYPH_NAMES[name]
  if (mapped !== undefined) return mapped
  // A-Z / a-z / 0-9 等的直接对照
  if (name.length === 1) return name.codePointAt(0)
  if (/^[A-Za-z]$/.test(name)) return name.codePointAt(0)
  return undefined
}

/** 一个字体：把字符串代码解码为文本。 */
interface FontDecoder {
  /** Type0/Identity-H 这类双字节编码。 */
  readonly twoByte: boolean
  /** 代码 → 文本（来自 /ToUnicode 与 /Differences）。 */
  readonly map: Map<number, string>
  /** 基础编码名（决定未映射字节的回退方式）。 */
  readonly baseEncoding: string | undefined
}
...
/** 为一个字体字典构建解码器。 */
function buildFontDecoder(doc: PdfDocument, fontDict: Map<string, PdfValue> | undefined): FontDecoder {
  const map = new Map<number, string>()
  ...
  if (toUnicode?.k === 'stream') {
    ...
    if (decoded !== undefined) {
      for (const [code, text] of parseToUnicode(latin1(decoded, 0, decoded.length))) map.set(code, text)
    }
  }

  const encodingValue = doc.resolve(fontDict?.get('Encoding'))
  let baseEncoding: string | undefined
  if (encodingValue?.k === 'name') baseEncoding = encodingValue.s
  else if (encodingValue?.k === 'dict') baseEncoding = doc.nameField(encodingValue.map, 'BaseEncoding')
  ...
  if (encodingValue?.k === 'dict') {
    ...
    if (differences?.k === 'arr') {
      ...
      for (const item of differences.items) {
        const resolved = doc.resolve(item)
        if (resolved?.k === 'num') { code = resolved.n; continue }
        if (resolved?.k === 'name') {
          const unicode = glyphToUnicode(resolved.s)
          if (unicode !== undefined) map.set(code, String.fromCodePoint(unicode))
          code += 1
        }
      }
    }
...
/** 用解码器把一段字节转成文本。 */
function decodeWithFont(bytes: Uint8Array, decoder: FontDecoder | undefined): string {
...
interface TextState {
  output: string
  decoder: FontDecoder | undefined
  /** 当前文本行的 y（Td/TD/Tm/T* 累计），用于判断是否**真的**换行。 */
  lineY: number
  /** 上一段文字写出时的 y。 */
  emittedY: number
  /** 是否已经写出过文字（决定是否抑制首个换行）。 */
  wrote: boolean
  /** BT 明确开启新文本块时的强制换行意图。 */
  forcedBreak: boolean
  /** 文字推进的 x（只用于同一行内的过大间隔判断）。 */
  x: number
  leading: number
}
 ...
 */
function extractFromContent(content: Uint8Array, resources: Map<string, PdfValue> | undefined, doc: PdfDocument, budget: { chars: number }): string {
  const cursor = new Cursor(content)
  const state: TextState = { output: '', decoder: undefined, lineY: 0, emittedY: 0, wrote: false, forcedBreak: false, x: 0, leading: 0 }
  const fonts = doc.resolveDict(resources?.get('Font'))
  const decoderCache = new Map<string, FontDecoder>()
  const operands: PdfValue[] = []

  const decoderFor = (name: string): FontDecoder | undefined => {
  ...
  /** 取最近的一个数字算子。 */
  const lastNumber = (): number | undefined => {
    for (let index = operands.length - 1; index >= 0; index -= 1) {
      const operand = operands[index]
      if (operand?.k === 'num') return operand.n
    }
    return undefined
  }
  ...
  while (!cursor.atEnd()) {
    ...
    if (byte === 0x2f) { operands.push({ k: 'name', s: parseName(cursor) }); continue }
    if (byte === 0x28) { operands.push({ k: 'str', bytes: parseLiteralString(cursor) }); continue }
    if (byte === 0x5b) { operands.push(parseValue(cursor)); continue }
    if (byte === 0x3c) {
      if (cursor.bytes[cursor.pos + 1] === 0x3c) { parseValue(cursor); continue } // 跳过内联字典
      operands.push({ k: 'str', bytes: parseHexString(cursor) })
      continue
    }
    if (byte === 0x5d || byte === 0x3e || byte === 0x29) { cursor.pos += 1; continue }

    const token = readRegular(cursor)
    if (token === '') { cursor.pos = before + 1; continue }
    ...
    switch (token) {
      ...
      case 'TL': {
        ...
        if (operand !== undefined) state.leading = operand
        break
      }
      case 'Td':
      case 'TD': {
        const numbers = operands.filter((item): item is { k: 'num'; n: number } => item.k === 'num')
        const ty = numbers[numbers.length - 1]?.n
        const tx = numbers[numbers.length - 2]?.n
        if (ty !== undefined && Number.isFinite(ty)) state.lineY += ty
        if (tx !== undefined && Number.isFinite(tx)) state.x += tx
        if (token === 'TD' && ty !== undefined) state.leading = -ty
        break
      }
      case 'Tm': {
        const numbers = operands.filter((item): item is { k: 'num'; n: number } => item.k === 'num')
      ...
      case '"': {
        ...
        if (operand?.k === 'str') emit(decodeWithFont(operand.bytes, state.decoder))
        break
      }
      case 'TJ': {
        ...
        if (array?.k === 'arr') {
          ...
          for (const item of array.items) {
            if (item.k === 'str') text += decodeWithFont(item.bytes, state.decoder)
            else if (item.k === 'num' && item.n <= TJ_FORWARD_WORD_GAP) text += ' '
          }
          emit(text)
        }
        break
    ...
    operands.length = 0
    if (budget.chars > 0 && state.output.length >= budget.chars) break
  }
...
function collectPages(doc: PdfDocument, maxPages: number): { pages: Map<string, PdfValue>[]; truncated: boolean } {
  const pages: Map<string, PdfValue>[] = []
  ...
  const walk = (dict: Map<string, PdfValue> | undefined, depth: number): void => {
    if (dict === undefined || depth > 64 || truncated) return
    const type = doc.nameField(dict, 'Type')
    if (type === 'Page') {
      if (pages.length >= maxPages) { truncated = true; return }
      pages.push(dict)
      return
    }
    const kids = doc.resolve(dict.get('Kids'))
    if (kids?.k !== 'arr') return
  ...
  walk(rootDict, 0)
  if (pages.length > 0) return { pages, truncated }
  ...
  for (const num of doc.objectNumbers()) {
    ...
    if (doc.nameField(dict, 'Type') === 'Page') pages.push(dict)
  }
  return { pages, truncated }
}

/** 读取一页的内容流字节（可能由多条流组成）。 */
function pageContent(doc: PdfDocument, page: Map<string, PdfValue>): Uint8Array[] {
  const contents = doc.resolve(page.get('Contents'))
  const streams: Uint8Array[] = []
  const push = (value: PdfValue | undefined): void => {
    const resolved = doc.resolve(value)
    if (resolved?.k !== 'stream') return
    const decoded = doc.decodeStream(resolved)
    if (decoded !== undefined) streams.push(decoded)
  }
  if (contents?.k === 'stream') push(contents)
  else if (contents?.k === 'arr') for (const item of contents.items) push(item)
  return streams
}
 ...
 */
export function extractPdfText(bytes: Uint8Array, limits: PdfExtractLimits = {}): ExtractedPdfText {
  ...
  if (bytes.length === 0) throw new Error('empty input: no PDF bytes were provided')
  if (bytes.length > maxBytes) throw new Error(`PDF is ${bytes.length} bytes, above the maxBytes limit of ${maxBytes}`)
  // 头部可能在前面有少量垃圾（邮件转发、前导空行），但必须是 PDF。
  const head = indexOfText(bytes, '%PDF-', 0)
  if (head < 0 || head > 1024) throw new Error('not a PDF: no %PDF- header was found in the first 1024 bytes')
  ...
  for (const page of collected.pages) {
    ...
    for (const stream of pageContent(doc, page)) {
      ...
      if (used + pageText.length >= maxChars) { truncated = true; break }
    }
    used += pageText.length
    if (pageText.trim().length > 0) chunks.push(pageText.replace(/[ \t]+$/gm, '').replace(/\n{3,}/g, '\n\n'))
  }

  const text = chunks.join('\n\n').replace(/^\n+/, '').replace(/\n+$/, '')
  if (text.length > maxChars) return { text: text.slice(0, maxChars), pages: reported, truncated: true }

...
function getUint16(bytes: Uint8Array, offset: number): number {
  return bytes[offset]! | (bytes[offset + 1]! << 8)
}

/** 小端读取 32 位无符号整数（无符号右移保证结果非负）。 */
function getUint32(bytes: Uint8Array, offset: number): number {
  return (bytes[offset]! | (bytes[offset + 1]! << 8) | (bytes[offset + 2]! << 16) | (bytes[offset + 3]! << 24)) >>> 0
}

/** 把条目名原始字节还原成字符串（ZIP 名字按 ASCII/Latin-1 处理）。 */
function nameFromBytes(bytes: Uint8Array, start: number, length: number): string {
  let out = ''
  for (let index = 0; index < length; index++) out += String.fromCharCode(bytes[start + index]!)
  return out
}

/** 校验区间 [start, start+length) 落在缓冲区内。 */
function assertRange(bytes: Uint8Array, start: number, length: number, what: string): void {
...
function findEndRecord(bytes: Uint8Array): number {
  ...
  for (let offset = bytes.length - END_RECORD_SIZE; offset >= lowest; offset--) {
    if (getUint32(bytes, offset) !== END_SIGNATURE) continue
    const commentLength = getUint16(bytes, offset + 20)
    // 注释长度必须正好铺满剩余字节，否则只是数据里碰巧出现的同名字节串。
    if (offset + END_RECORD_SIZE + commentLength === bytes.length) return offset
  }
 ...
 * 结构损坏、超出上限都会抛 Error。
 */
export function readZip(bytes: Uint8Array, limits?: ZipReadLimits): Map<string, ZipEntryData> {
  if (!(bytes instanceof Uint8Array)) throw new Error('readZip 需要 Uint8Array 输入')
  ...
  const result = new Map<string, ZipEntryData>()
  ...
  for (let index = 0; index < entriesTotal; index++) {
    ...
    if ((flags & 0x0001) !== 0) throw new Error(`ZIP 条目 ${JSON.stringify(name)} 已加密，不支持读取`)
 ...
 */
function inflateEntry(name: string, raw: Uint8Array, uncompressedSize: number, maxEntryBytes: number): Uint8Array {

 ...
 */
function resolveWritePolicy(ctx, exec) {
 ...
 */
export async function resolveDocumentTarget(ctx, exec, rawPath, signal) {
...
export async function readTargetBytes(ctx, target, signal, maxBytes = MAX_READ_BYTES) {
...
export async function targetExists(ctx, target, signal) {
    const info = await ctx.fs.stat(target.target, signal);
 ...
 */
export async function writeTargetText(ctx, exec, target, content, signal) {
...
export function bytesToAsciiText(bytes) {
    let out = '';
    for (let i = 0; i < bytes.length; i += 1) {
        const byte = bytes[i];
        if (byte > 0x7f)
            throw new Error(`生成物第 ${i} 字节为 0x${byte.toString(16)}，不是纯 ASCII，无法经文本通道写出`);
 ...
 */
export function describeWriteFailure(error, display) {

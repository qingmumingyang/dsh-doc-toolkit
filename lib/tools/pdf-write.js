...
import { bytesToAsciiText, writeTargetText } from '../utils/fs-channel.js';
/** 单个字体候选文件的大小上限（TTC 集合较大，给足空间但仍有界）。 */
const MAX_FONT_BYTES = 64 * 1024 * 1024;
/** 字节是否全是 ASCII（决定流能否直接原样写入）。 */
function isPureAscii(data) {
...
export function parseFont(buf, fontIndex = 0) {
    ...
    if (numGlyphs === 0)
        throw new Error('字体 maxp 表报告 0 个字形（表偏移可能不正确）');
...
export function selectBestFont(buf, cps) {
    if (buf.subarray(0, 4).toString('latin1') === 'ttcf') {
        ...
        if (!best)
            throw new Error('TTC 中无可用 TrueType 字体（没有子字体覆盖所需字符）');
...
function cjkFontCandidates(extraFonts = []) {
 ...
 */
export async function writePDF(ctx, exec, target, content, signal, extraFonts = []) {

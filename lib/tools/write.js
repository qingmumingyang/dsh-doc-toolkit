...
import { bytesToAsciiText, describeWriteFailure, resolveDocumentTarget, writeTargetText } from '../utils/fs-channel.js';
...
export function buildDocxPackage(content) {
...
export function buildXlsxPackage(content) {
    ...
    if (Array.isArray(content.rows)) {
        ...
        const rows = content.rows.map((row) => row.slice());
    ...
    if (Array.isArray(content.data)) {
        ...
        const rows = [headers, ...data.map((row) => headers.map((h) => (row?.[h] === null || row?.[h] === undefined ? '' : row[h])))];
...
export function buildCsvText(content) {
    if (Array.isArray(content.rows)) {
        const rows = content.rows;
        return { text: rows.map((row) => row.map(escapeCSVField).join(',')).join('\r\n'), rowCount: rows.length };
    }
    if (Array.isArray(content.data)) {
        const data = content.data;
        const headers = [...new Set(data.flatMap((row) => Object.keys(row)))];
...
export function registerWriteTools(ctx, config = {}) {

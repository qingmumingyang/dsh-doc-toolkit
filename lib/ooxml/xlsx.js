...
function decodeExcelEscapes(value) {
    if (value.indexOf('_x') < 0)
        return value;
    return value.replace(/_x([0-9A-Fa-f]{4})_/g, (whole, hex) => {
...
function cellValue(cellXml, type, shared) {
    if (type === 's') {
        ...
        if (!Number.isFinite(index) || index < 0 || index >= shared.length)
            return '';
        return shared[index];
    }
    if (type === 'inlineStr') {
...
function parseSheetRows(xml, shared) {
    ...
    for (let index = 0; index < rows.length; index++) {
        ...
        if (row === undefined) {
            rows[index] = new Array(width).fill('');
            continue;
        }
 ...
 */
export function readXlsxSheets(zip) {
    const workbookXml = partText(zip, 'xl/workbook.xml');
    if (workbookXml === undefined)
        throw new Error('XLSX 缺少工作簿部件 xl/workbook.xml');
...
export function buildXlsx(input) {
    ...
    const sourceRows = (input.rows ?? []).filter((row) => Array.isArray(row));
    let maxColumns = 0;
...
function encodeAscii(xml) {
    ...
    for (let index = 0; index < xml.length; index++) {
        ...
        if (code > 0x7f)
            throw new Error(`XLSX 模板出现非 ASCII 字符（位置 ${index}），请改用 escapeXmlText 转义`);
        bytes[index] = code;
    }
    return bytes;
}

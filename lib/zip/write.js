 ...
 */
function feasibleLocalExtra(end) {
    if (safeField(end))
        return 0;
    for (let total = MIN_EXTRA; total <= MAX_LOCAL_EXTRA; total += 1) {
        if (safeField(end + total))
            return total;
    }
    return undefined;
}
 ...
 */
function feasibleCentralExtra(base) {
    if (safeField(base))
        return 0;
    for (const candidate of centralExtraCandidates()) {
        if (candidate !== 0 && safeField(base + candidate))
            return candidate;
    }
    return undefined;
}
 ...
 * 边界，从而覆盖单值无法到达的安全区间。
 */
function centralExtraCandidates() {
    const list = [0];
    for (let value = MIN_EXTRA; value <= MAX_LOCAL_EXTRA; value += 1)
        list.push(value);
    for (let high = 1; high <= 0x7f; high += 1) {
        const value = (high << 8) | MIN_EXTRA;
        if (safeWord(value) && safeWord(value - MIN_EXTRA))
            list.push(value);
    }
    return list;
}
 ...
 *    因此方案 3 只在极端情况下生效。
 */
function tuneCentralDirectory(planned) {
    const base = centralDirectorySize(planned);
    if (safeField(base))
        return { lastExtra: 0, secondLastExtra: 0, signatureData: 0 };
    const candidates = centralExtraCandidates();
    for (const first of candidates) {
        if (safeField(base + first))
            return { lastExtra: first, secondLastExtra: 0, signatureData: 0 };
    }
    if (planned.length >= 2) {
        for (const first of candidates) {
...
/** 判断字节是否全为 ASCII。 */
function assertAsciiBytes(data, what) {
...
function planEntries(entries) {
    ...
    for (const entry of entries) {
        if (entry.name.length === 0)
            throw new Error('ZIP 条目名不能为空');
        assertAsciiBytes(Uint8Array.from(entry.name, (ch) => ch.charCodeAt(0)), `ZIP 条目名 ${JSON.stringify(entry.name)}`);
        if (!safeWord(entry.name.length))
            throw new Error(`ZIP 条目名过长：${entry.name.length}`);
    ...
    if (planned.length > 0) {
        ...
        if (tuning.lastExtra > 0)
            planned[planned.length - 1].centralExtra = tuning.lastExtra;
...
function centralDirectorySize(entries, signatureData = 0) {
    let total = 0;
    for (const entry of entries) {
        total += CENTRAL_HEADER_BYTES + entry.name.length + entry.centralExtra;
    }
    if (signatureData > 0)
        total += SIGNATURE_RECORD_BYTES + signatureData;
    return total;
}
/**
...
export function buildStoredZip(entries) {
    ...
    if (entries.length > MAX_ENTRIES)
        throw new Error(`ZIP 条目数 ${entries.length} 超过上限 ${MAX_ENTRIES}（未实现 ZIP64）`);
    const { entries: planned, signatureData } = planEntries(entries);
    const names = new Set();
    for (const entry of planned) {
        if (names.has(entry.name))
            throw new Error(`ZIP 条目名重复：${entry.name}`);
    ...
    for (const entry of planned) {
        chunks.push(ascii(dword(LOCAL_SIGNATURE)
            + word(VERSION)
        ...
        if (entry.pad > 0)
            chunks.push(newlines(entry.pad));
    }
    // 中央目录
    ...
    for (const entry of planned) {
        chunks.push(ascii(dword(CENTRAL_SIGNATURE)
    ...
    chunks.push(ascii(dword(END_SIGNATURE)
        + word(0) // 本磁盘号
        + word(0) // 中央目录起始磁盘号
    ...
    if (!safeField(directoryOffset))
        throw new Error('ZIP 中央目录偏移不是 ASCII 安全值');
    if (!safeField(directorySize))
        throw new Error('ZIP 中央目录大小不是 ASCII 安全值');
    ...
    for (let index = 0; index < out.length; index += 1) {
        if (out[index] > 0x7f)
            throw new Error(`ZIP 输出第 ${index} 字节不是 ASCII（0x${out[index].toString(16)}）`);
    }
    return out;
}
